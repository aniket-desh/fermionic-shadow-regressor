...
def majorana_shadow_estimates_full_system(outcomes, k: int = 2) -> dict:
    """
    ...
    for permutation, bit_string in outcomes:
        ...
        for diag_op, target_op in estimated_ops:
            ...
            operator"""
            inverse_permutation_on_predicted_op = [
                inverse_permutation[i] for i in target_op
            ]
...
def majorana_shadow_estimates_spin_adapted(
    outcomes,
...
def compute_expectation_values_from_majorana_observables(
    majorana_expectations: dict, observables: Union[dict, Sequence[dict]]
