...
# Lazy imports — cirq is only needed for circuit-building functions,
# not for the matrix decomposition functions used by the numpy-based pipeline.
cirq = None  # type: ignore


def _ensure_cirq():
    global cirq
    if cirq is None or cirq is False:
        import importlib
...
def givens_rotate(operator, givens_rotation, i, j, which="row"):
    """Apply a Givens rotation to rows/cols i,j of operator (in-place).

    Inlined from openfermion.linalg.givens_rotations to avoid the
    openfermion dependency for decomposition-only usage.
    """
    if which == "row":
        ...
        operator[:, i] = (
            givens_rotation[0, 0] * col_i + givens_rotation[0, 1].conj() * col_j
        )
        operator[:, j] = (
            givens_rotation[1, 0] * col_i + givens_rotation[1, 1].conj() * col_j
        )
    else:
        raise ValueError('"which" must be "row" or "col".')


def optimal_gaussian_circuit(
    qubits: Sequence, orthogonal_matrix: np.ndarray
) -> Iterable:
    """
    ...
    left_rotations, right_rotations, signs = majorana_block_decomposition(
        orthogonal_matrix
    )
...
def optimal_gaussian_circuit_spin_block_diag(
    up_qubits: Sequence,
    down_qubits: Sequence,
...
def majorana_block_decomposition(
...
def block_ortho_transform(
    operator: np.ndarray, orthogonal_matrix: np.ndarray, p: int, q: int, which="row"
):
    """
...
def real_givens_matrix(theta: float) -> np.ndarray:
    ...
    c = np.cos(theta)
    s = np.sin(theta)
    gmat = np.array([[c, -s], [s, c]])

    return gmat


def standard_givens_decomposition(
    orthogonal_matrix: np.ndarray,
...
def standard_majorana_givens_circuit(
    qubits: Sequence, orthogonal_matrix: np.ndarray, index_offset: int = 0
) -> Iterable:
    """
    Constructs a Cirq circuit which implements the fermionic Gaussian
    unitary transformation given by the 2n x 2n orthogonal matrix, using
...
def majorana_rotation_gate(qubits: Sequence, p: int, q: int, theta: float) -> Iterable:
    """
...
def majorana_sign_gates(qubits: Sequence, signs: Iterable) -> Iterable:
    """
...
def embed_unitary_into_orthogonal_matrix(unitary: np.ndarray) -> np.ndarray:
    """
    ...
    for p in range(n_orbitals):
        ...
        orthogonal_matrix[2 * p : 2 * p + 2, 2 * p : 2 * p + 2] = np.array(
            [[real_part, -imag_part], [imag_part, real_part]]
        )
