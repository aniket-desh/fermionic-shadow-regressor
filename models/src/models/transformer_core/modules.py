...
Single source of truth for the attention/FFN/embedding/normalization modules
that were previously duplicated (byte-for-byte, modulo cosmetics) between
``src.models.transformer`` and ``src.models.gctransformer``. Both packages
re-export these names.

``PositionalEncoding2D`` is only used by the generative transformer's 2D-lattice
variant; it is hosted here as a generic building block.
"""
