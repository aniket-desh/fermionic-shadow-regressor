#!/bin/bash
# =======================================================================
# Resubmit v10 training + eval only (datagen already done)
#
# Job 454758 died at AdamW init with:
#   ModuleNotFoundError: No module named 'mpmath'
# (torch._dynamo → sympy → mpmath, missing in the venv)
#
# Before running, on Trillium:
#   source ~/envs/gqs/bin/activate && pip install mpmath
#
# Usage (on Trillium):
#   cd $SCRATCH/generative-quantum-states
#   bash slurm/regression_v10_resubmit.sh
# =======================================================================
set -euo pipefail
...
if [ ! -f "${DATA_PATH}" ]; then
  ...
  echo "Run slurm/regression_v10.sh first (or pull from a prior datagen)."
  exit 1
fi

echo "=== Regression v10 resubmit (train + eval) ==="
echo "  tag:     ${TAG}"
