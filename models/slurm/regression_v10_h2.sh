#!/bin/bash
# =======================================================================
# Regression v10 on H2: HF orbital features + dense full-range sampling
#
# Mirror of slurm/regression_v10.sh with --n_atoms 2. Same model
# architecture, same training schedule, same R grid, same t_max=300.
# Validates whether the v10 recipe (HF orbital-energy freq_net input,
# dR=0.01 dense grid) generalizes off H4 onto a single-band test case.
#
# H2 specifics:
#   - 2 spatial orbitals (STO-3G) -> 4 spin-orbitals -> 4 modes
#   - dissociation curve smooth; no multi-reference dip to chase
#   - freq_net input dim = n_spatial_orb = 2 (was 4 for H4)
#
# Usage: cd $SCRATCH/generative-quantum-states && bash slurm/regression_v10_h2.sh
# =======================================================================
set -euo pipefail

TAG="${1:-h2_regress_v10}"
...
echo "=== Regression v10 on H2 (HF orbital features + dense full-range) ==="
echo "  tag:     ${TAG}"
